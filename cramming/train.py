...
def parse_arguments():
    ...
    parser.add_argument('--save_path', type=str, default=str(SAVE_DIR), help='path to save experiments')
    ...
    parser.add_argument('--vocab_path', type=str, default=str(VOCAB_PATH),
                        help='path to vocab file used when --shuffled is set')
...
def run_single_experiment(N_mem_tokens, text_sample, max_length, num_iterations, sample_idx, run_idx,
                          ...
                          weight_decay, early_stopping_patience=2000, shuffled=False, vocab_path=VOCAB_PATH):
    ...
    # Treat near-1.0 accuracies as success to avoid float rounding issues
    SUCCESS_EPS = 1e-6
    ...
    for _ in progress_bar:
        ...
        # If accuracy effectively reaches 1.0, stop early
        if best_accuracy >= 1.0 - SUCCESS_EPS:
            best_accuracy = 1.0
            break
...
def main():
    ...
    for max_length in args.max_length:
        for N_mem_tokens in args.N_mem_tokens:
            ...
            save_path = Path(args.save_path) / args.model_name
            ...
            for sample_idx, sample in enumerate(samples):
                for run in range(num_runs):
                    result = run_single_experiment(N_mem_tokens, sample, max_length, args.num_iterations, sample_idx,
                                                   ...
                                                   args.early_stopping_patience, args.shuffled, args.vocab_path)
