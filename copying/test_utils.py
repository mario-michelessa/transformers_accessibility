...
def evaluation(args, model, tokenizer, TO_TOKEN):
    ...
    lengths_override = getattr(args, "eval_lengths", None)
    lengths = np.array(lengths_override if lengths_override is not None else np.arange(args.min_eval_len, args.max_eval_len))
